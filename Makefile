...
image:
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=akshithachittanoori/testt:US-620855-9-jdk11 -t $(IMAGE_NAME):US-620855-9-jdk11 . # Build image and automatically tag it as latest on jdk17
...
test: image
	...
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=akshithachittanoori/testt:US-620855-9-jdk11 -t qualitytest . --target qualitytest
	# Execute test cases
...
# 	docker push $(IMAGE_NAME):$(VERSION)-jdk17
	docker push $(IMAGE_NAME):US-620855-9-jdk11

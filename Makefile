...
image:
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=akshithachittanoori276/testt:US-620855test-11-ubuntu22.04 -t $(IMAGE_NAME):US-620855test-11-ubuntu22.04 . # Build image and automatically tag it as latest on jdk17
...
test: image
	...
	docker build --build-arg VERSION=$(VERSION) --build-arg BASE_TOMCAT_IMAGE=akshithachittanoori276/testt:US-620855test-11-ubuntu22.04 -t qualitytest . --target qualitytest
	# Execute test cases
	container-structure-test test --image qualitytest --config tests/pega-web-ready-testcases.yaml
	container-structure-test test --image $(IMAGE_NAME):US-620855test-11-ubuntu22.04 --config tests/pega-web-ready-release-testcases.yaml
# 	container-structure-test test --image $(IMAGE_NAME) --config tests/pega-web-ready-release-testcases_jdk17_version.yaml
	container-structure-test test --image $(IMAGE_NAME):US-620855test-11-ubuntu22.04 --config tests/pega-web-ready-release-testcases.yaml
	container-structure-test test --image $(IMAGE_NAME):US-620855test-11-ubuntu22.04 --config tests/pega-web-ready-release-testcases_jdk11_version.yaml
...
# 	docker push $(IMAGE_NAME):$(VERSION)-jdk17
	docker push $(IMAGE_NAME):US-620855test-11-ubuntu22.04
